...
class TwitchBaseIE(InfoExtractor):
    _VALID_URL_BASE = r'https?://(?:www\.|secure\.)?twitch\.tv'
...
#            '%s/api/videos/%s%s' % (self._API_BASE, self._ITEM_SHORTCUT, item_id), item_id,
    ...
    def _extract_info(self, info):
        ...
        } for image in info.get('thumbnails')]
        if info.get('preview'): 
            if {'url': info.get('preview'), 'filename': info.get('preview').rsplit('/', 1)[1]} not in thumbnails: 
                thumbnails.append({'url': info.get('preview'), 'filename': info.get('preview').rsplit('/', 1)[1]})# if info.get('preview')
        if info.get('animated_preview'): thumbnails.append({'url': info.get('animated_preview'), 'filename': info.get('animated_preview').rsplit('/', 1)[1]})# if info.get('animated_preview')
        #print thumbnails
        return {
            'id': info['_id'],
...
#            '%s/api/vods/%s/access_token' % (self._API_BASE, item_id), item_id,
        ...
        formats = self._extract_m3u8_formats(
            ...
            item_id, 'ts' if self._downloader.params.get('hls_use_mpegts', False) else 'mp4', entry_protocol='m3u8_native')
...
#            [self.url_result(entry) for entry in orderedSet(entries)],
            [self.url_result(entry['url'], TwitchVodIE.ie_key(), entry['_id']) for entry in orderedSet(entries)],
...
#            '%s/api/channels/%s/access_token' % (self._API_BASE, channel_id), channel_id,
        ...
        formats = self._extract_m3u8_formats(
            ...
            channel_id, 'ts' if self._downloader.params.get('hls_use_mpegts', False) else 'mp4')
        self._prefer_source(formats)
